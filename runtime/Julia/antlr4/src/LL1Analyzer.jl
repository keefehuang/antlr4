...
#/
function getDecisionLookahead(LL1::LL1Analyzer, s::ATNState)
    if isnothing(s)
        return nothing
    end

    count = length(s.transitions)
    look = Vector{Set}(undef, count)
    for alt in 1:count
        look[alt] = Set()
        lookBusy = Set()
        seeThruPreds = false # fail to get lookahead upon pred
        _LOOK(LL1, transition(s, alt).target, nothing, PredictionContext.EMPTY,
                look[alt], lookBusy, Set(), seeThruPreds, False)
        # Wipe out lookahead for this alternative if we found nothing
        # or we had a predicate when we !seeThruPreds
        if length(look[alt])==0 || HIT_PRED in look[alt]
            look[alt] = nothing
        end
    end
    return look
...
# specified {@code ctx}.
#/
function LOOK(LL1::LL1Analyzer, s::ATNState, stopState::nATNState=nothing, ctx:nRuleContext=nothing)
    r = IntervalSet()
    seeThruPreds = true # ignore preds; get all lookahead
    lookContext = ifelse(isnothing(ctx), PredictionContextFromRuleContext(s.atn, ctx), nothing)
    _LOOK(LL1, s, stopState, lookContext, r, set(), set(), seeThruPreds, true)
    return r
end
...
# is {@code null}.
#/
function _LOOK(LL1::LL1Analyzer, s::ATNState, stopState::ATNState , ctx::PredictionContext, look::IntervalSet, lookBusy::Set,
                    calledRuleStack::Set, seeThruPreds::Boolean, addEOF::Boolean)
    c = ATNConfig(s, 0, ctx)
    ...
    if s == stopState
        if isnothing(ctx)
            addOne!(look, Token.EPSILON)
            return
        elseif isempty(ctx) && addEOF
            look.addOne!(Token.EOF)
            return
        end
    end

    if s isa RuleStopState
        if isnothing(ctx)
            addOne!(look, Token.EPSILON)
            return
        elseif isempty(ctx) && addEOF
            addOne!(look, Token.EOF)
            return
        end
        ...
        if ctx != PredictionContext.EMPTY
            ...
            try
                ...
                for i in 1:length(ctx)
                    returnState = LL1.atn.states[getReturnState(ctx, i)]
                    _LOOK(LL1, returnState, stopState, getParent(ctx, i), look, lookBusy, calledRuleStack, seeThruPreds, addEOF)
                end
            finally
                if removed
                    add(calledRuleStack, s.ruleIndex)
                end
            end
            return
        end
    ...
    for t in s.transitions
        if t isa RuleTransition
            ...
            newContext = create(ctx, t.followState.stateNumber)

            try
                add!(calledRuleStack, t.target.ruleIndex)
                _LOOK(LL1, t.target, stopState, newContext, look, lookBusy, calledRuleStack, seeThruPreds, addEOF)
            finally
                calledRuleStack.remove(t.target.ruleIndex)
            end
        elseif t isa AbstractPredicateTransition
            if seeThruPreds
                _LOOK(LL1, t.target, stopState, ctx, look, lookBusy, calledRuleStack, seeThruPreds, addEOF)
            else
                addOne!(look, HIT_PRED)
            end
        elseif t.isEpsilon
            _LOOK(LL1, t.target, stopState, ctx, look, lookBusy, calledRuleStack, seeThruPreds, addEOF)
        elseif t isa WildcardTransition
            addRange!(look, Token.MIN_USER_TOKEN_TYPE:self.atn.maxTokenType)
        else
            set_ = t.label
            if !isnothing(set_)
                if t isa NotSetTransition
